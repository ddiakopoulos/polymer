...
layout(binding = 1, std140) uniform PerView
{
    ...
    mat4 u_viewProjMatrix;
    vec4 u_eyePos;
    vec4 u_zBufferParams;     // x = 1-far/near, y = far/near, z = x/far, w = y/far
    vec4 u_projectionParams;  // x = 1 or -1 (-1 if projection is flipped), y = near plane, z = far plane, w = 1/far plane
};
...
float linear_01_depth(in float z, in vec4 zBufferParams) 
{
    return (1.0 / (zBufferParams.x * z + zBufferParams.y));
}
>>>>>>> 0aca943... added z buffer params and projection params per standard Unity shaders for better interop
