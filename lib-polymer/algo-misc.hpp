...
namespace polymer
{
    ...
    class cantor_set
    {
        std::vector<segment_3d> lines; // initial

        std::vector<segment_3d> next(const segment_3d & l) const
        {
            const float3 p0 = l.a;
            const float3 pn = l.b;
    ...
    public:
        ...
        std::vector<segment_3d> step()
        {
            std::vector<segment_3d> recomputedSet;
            for (auto & l : lines)
            {
                std::vector<segment_3d> nextLines = next(l); // split
