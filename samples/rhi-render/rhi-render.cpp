...
struct sample_rhi_render final : public polymer_app
{
...
sample_rhi_render::~sample_rhi_render() {}

void sample_rhi_render::on_window_resize(int2 size) {}

void sample_rhi_render::on_input(const app_input_event & event)
{
...
void sample_rhi_render::on_update(const app_update_event & e)
{
    int width, height;
    glfwGetWindowSize(window, &width, &height);
}

void sample_rhi_render::on_draw()
{
    glfwMakeContextCurrent(window);
    ...
    {
        modelPose.orientation = normalize(modelPose.orientation * arcball->currentQuat);
    }
