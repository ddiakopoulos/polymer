...
namespace inlined_vector_internal
{
    ...
    using IsMemcpyOk = polymer::conjunction<std::is_same<AllocatorType, std::allocator<ValueType>>,
                                            std::is_trivially_copy_constructible<ValueType>,
                                            std::is_trivially_copy_assignable<ValueType>,
                                            std::is_trivially_destructible<ValueType>>;
    ...
    public:
        ...
        ~ConstructionTransaction()
        {
            if (DidConstruct())
            {
                inlined_vector_internal::DestroyElements(std::addressof(GetAllocator()), GetData(), GetSize());
            }
        }
    ...
    public:
        ...
        {
            return static_cast<size_type>(std::get<1>(metadata_));
        }
        ...
        {
            return reinterpret_cast<pointer>(std::addressof(data_.inlined.inlined_data[0]));
        }

        const_pointer GetInlinedData() const
        {
            return reinterpret_cast<const_pointer>(std::addressof(data_.inlined.inlined_data[0]));
        }
        ...
        {
            return GetIsAllocated() ? StorageView {GetAllocatedData(), GetSize(), GetAllocatedCapacity()} : StorageView {GetInlinedData(), GetSize(), GetInlinedCapacity()};
        }

        allocator_type * GetAllocPtr()
        {
            return std::addressof(std::get<0>(metadata_));
        }
        ...
        {
            GetSizeAndIsAllocated() = (size << 1) | static_cast<size_type>(GetIsAllocated());
        }
        ...
        void DeallocateIfAllocated()
        {
            if (GetIsAllocated())
            {
                AllocatorTraits::deallocate(*GetAllocPtr(), GetAllocatedData(), GetAllocatedCapacity());
            }
        }
    ...
    auto Storage<T, N, A>::Insert(const_iterator pos, ValueAdapter values, size_type insert_count) -> iterator
    {
        ...
        if (new_size > storage_view.capacity)
        {
            ...
            inlined_vector_internal::ConstructElements( GetAllocPtr(), new_data + insert_end_index, &move_values, storage_view.size - insert_index);
        ...
        else
        {
            ...
            nonstd::span<value_type> move_construction = {
                storage_view.data + move_construction_destination_index,
                new_size - move_construction_destination_index};

            pointer move_assignment_values = storage_view.data + insert_index;
            nonstd::span<value_type> move_assignment = {
                storage_view.data + insert_end_index,
                move_construction_destination_index - insert_end_index};

            nonstd::span<value_type> insert_assignment = {move_assignment_values,
                                                        move_construction.size()};

            nonstd::span<value_type> insert_construction = {
            ...
            for (pointer destination = move_assignment.data() + move_assignment.size(), last_destination = move_assignment.data(), source = move_assignment_values + move_assignment.size(); ;)
            {
                --destination;
                --source;
    ...
    auto Storage<T, N, A>::Swap(Storage * other_storage_ptr) -> void
    {
        ...
        else
        {
            ...
            try
            {
                inlined_vector_internal::ConstructElements(inlined_ptr->GetAllocPtr(), allocated_ptr->GetInlinedData(), &move_values, inlined_ptr->GetSize());
            }
            catch (const std::exception & e)
            {
...
class inlined_vector
{
    ...
    template <typename Iterator>
    using EnableIfAtLeastForwardIterator  = std::enable_if_t<inlined_vector_internal::IsAtLeastForwardIterator<Iterator>::value>;

    template <typename Iterator>
    using DisableIfAtLeastForwardIterator = std::enable_if_t< !inlined_vector_internal::IsAtLeastForwardIterator<Iterator>::value>;
...
public:
    ...
    inlined_vector(std::initializer_list<value_type> list, const allocator_type & alloc = allocator_type()) : inlined_vector(list.begin(), list.end(), alloc) {}
    ...
    inlined_vector(inlined_vector && other) noexcept : storage_(*other.storage_.GetAllocPtr())
    {
        ...
        else
        {
            ...
            inlined_vector_internal::ConstructElements( storage_.GetAllocPtr(), storage_.GetInlinedData(), &other_values, other.storage_.GetSize());
    ...
    reference at(size_type i)
    {
        if ((i >= size()))
        {
            throw std::invalid_argument("`inlined_vector::at(size_type)` failed bounds check");
        }
    ...
    const_reference at(size_type i) const
    {
        if ((i >= size()))
        {
            throw std::invalid_argument("`inlined_vector::at(size_type) const` failed bounds check");
        }
    ...
    iterator insert(const_iterator pos, ForwardIterator first, ForwardIterator last)
    {
        ...
        if ((first != last))
        {
            return storage_.Insert(pos, IteratorValueAdapter<ForwardIterator>(first), std::distance(first, last));
        }
        else
        {
            return const_cast<iterator>(pos);
